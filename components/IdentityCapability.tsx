  ...
  return (
    <section id="whoweare" className="py-24 md:py-32 bg-white relative">
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        <div className="text-center mb-20 scroll-reveal reveal-bottom">
        ...
        <div className="grid md:grid-cols-1 lg:grid-cols-3 gap-8 mb-24">
          {capabilities.map((item, idx) => (
            <div 
              key={idx} 
              className="stagger-reveal group p-8 md:p-12 rounded-[2rem] border border-slate-100 bg-slate-50 hover:bg-white hover:border-[#003c95]/20 hover:shadow-xl transition-all duration-500"
              style={{ transitionDelay: `${idx * 150}ms` }}
            >
              <div className="w-12 h-12 bg-white rounded-xl flex items-center justify-center text-[#003c95] shadow-sm group-hover:bg-[#003c95] group-hover:text-white transition-all mb-8">
                {item.icon}
              </div>
              <h4 className="text-2xl font-bold text-slate-900 mb-4 group-hover:text-[#003c95] transition-colors">{item.title}</h4>
              <p className="text-slate-500 text-base font-medium leading-relaxed">{item.desc}</p>
            </div>
          ))}
        </div>

        <div className="scroll-reveal reveal-bottom flex flex-col items-center">
          <div className="flex flex-col md:flex-row items-center gap-10 md:gap-16">
            <p className="text-2xl md:text-4xl font-black text-slate-900 tracking-tighter leading-tight text-center md:text-left">
              "WE DON’T PROVIDE PEOPLE. <br className="hidden md:block" /> 
              WE PROVIDE <span className="text-[#003c95]">PREDICTABLE EXECUTION."</span>
            </p>
            
            <div className="flex items-center gap-4 px-6 py-3 bg-slate-50 rounded-full border border-slate-100 shadow-sm transition-all hover:shadow-md hover:border-slate-200">
              <span className="w-2 h-2 rounded-full bg-green-500 animate-pulse"></span>
              <span className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] whitespace-nowrap">
                Active Ops: <span className="text-slate-900">Addis Ababa, Ethiopia</span>
              </span>
            </div>
          </div>
          <div className="h-1 w-20 bg-[#003c95] mt-12 rounded-full"></div>
        </div>
      </div>
    </section>
  );
